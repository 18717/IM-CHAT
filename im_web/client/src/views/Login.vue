<template>
  ...
  <el-form
    ...
    <h2 class="login-title">IM-CHAT</h2>
...
export default {
  ...
  head: {
    title: "IM-CHAT",
    meta: [
      {name: 'description', content: '客户端登录'}
    ]
  },
  data() {
    return {
  ...
  methods: {
    toLogin() {
      this.loading = true;
      this.postRequest('/login/client', this.loginForm).then(resp => {
        if (resp) {
    ...
    updateCaptcha() {
      this.captchaUrl = '/captcha/client/login?username=' + this.loginForm.username + '?time=' + new Date();
    },
